...
if __name__ == "__main__":
    ...
    ### ------- Repeat but hold latency constant     
    for eff in ['detections', 'measures', 'both']:
        #fig, ax1 = plt.subplots(layout='constrained', figsize=(3.25,2.36))
        fig, ax1 = plt.subplots(layout='constrained', figsize=(2.5,1.8))
        # fig, ax1 = plt.subplots(layout='constrained', figsize=(1.6, 1.8))
        ...
        snr_db = 20*np.log10(snr_exp_vec(N/H, H))
        ax1.semilogx(H, snr_db, color='black')
        ...
        if eff=='detections':
            ax2.set_ylabel(r'$SNR^2/D$', color = color)
            ax2.semilogx(H, snr_exp_vec((N/H), H)**2/(N/H*(1-np.exp(-H))), color = color)
        elif eff=='measures':
            ax2.set_ylabel(r'$SNR^2/W$', color = color)
            ax2.semilogx(H, snr_exp_vec((N/H),H)**2/((N/H)), color = color)
        elif eff == 'both':
            # ax2.set_ylabel(r'$SNR^2/D$', color = color, fontsize=fz)
            ax2.semilogx(H, snr_exp_vec((N/H),H)**2/((N/H)*(1-np.exp(-H))), color = color, linestyle='dashed')
            color = 'tab:blue'                        
            # ax2.set_ylabel(r'$SNR^2/W$', color = color, fontsize=fz)
            ax2.semilogx(H, snr_exp_vec((N/H),H)**2/((N/H)), color = color, linestyle='-.')
        ...
        my_savefig(fig, figure_dir, f'snr_constantTL_{eff}_eff')
